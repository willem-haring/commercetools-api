export { Config } from "./lib/Config.ts";
export type { iConfig } from "./lib/interface/iConfig.ts";
export {loglevel} from "./lib/interface/iLogger.ts"

export {importClient, ApiRoot as importRoot} from "./lib/importClient.ts"
export { importsdkapi } from "./lib/importClient.ts"
export { sdkClient, ApiRoot as sdkRoot } from "./lib/sdkClient.ts"
export { plafformsdk } from "./lib/sdkClient.ts"
export * as sdkclient from "npm:@commercetools/sdk-client-v2"

export {sdk} from "./lib/sdk.ts"
export {importsdk } from "./lib/importsdk.ts"
